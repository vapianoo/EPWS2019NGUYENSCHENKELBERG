...
public class Camera_movement : MonoBehaviour
{
    ...
    void moveCamera(float distance)
    {
        ...
        {
            transform.position = Vector3.MoveTowards(transform.position, endPosition + (endPosition_next - endPosition) / Mathf.Pow(distance,2), 0.05f);
            transform.LookAt(lookAtPosition);
        }
    ...
    void moveLookAtTarget(float distance)
    {
        ...
        {
            lookAtPosition = Vector3.MoveTowards(lookAtPosition, lookAtTarget + (lookAtTarget_next - lookAtTarget) / Mathf.Pow(distance, 2), 0.05f);
        }
